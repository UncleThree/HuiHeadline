...
- (void)wm_setup {
    //eyuxin
    _titleSizeSelected  = 19.0f;
    _titleSizeNormal    = 18.0f;
    _titleColorSelected = [UIColor colorWithRed:230/255.0 green:53/255.0 blue:40/255.0 alpha:1];
    _titleColorNormal   = [UIColor colorWithRed:51/255.0 green:51/255.0 blue:51/255.0 alpha:1];
...
- (void)wm_calculateSize {
    ...
    if ([self.dataSource respondsToSelector:@selector(pageController:preferredFrameForMenuView:)]) {
        ...
        _menuViewFrame = CGRectMake(0, 0, self.view.frame.size.width - segmentHeight, segmentHeight);
    }
...
- (void)wm_addMenuView {
    ...
    menuView.backgroundColor = [UIColor colorWithRed:245/255.0 green:245/255.0 blue:245/255.0 alpha:1];
...
//eyuxin change
- (void)wm_layoutChildViewControllers:(BOOL)delayInit {
    ...
    for (int i = 0; i < self.childControllersCount; i++) {
        ...
        if (!vc ) {
            if ([self wm_isInScreen:frame] ) {
                ...
                [self wm_initializedControllerWithIndexIfNeeded:i delayInit:delayInit];
            }
        } else if (i <= left || i >= right) {
            if (![self wm_isInScreen:frame]) {
                [self wm_removeViewController:vc atIndex:i];
...
//change
- (void)wm_addAddVButton {
    
    UIButton *add = [[UIButton alloc] initWithFrame:CGRectMake(UIScreen.mainScreen.bounds.size.width - segmentHeight, _menuViewFrame.origin.y, segmentHeight, segmentHeight)];
    [add setImage:[[UIImage imageNamed:@"更多"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] forState:UIControlStateNormal];
    [add addTarget:self action:@selector(channelList) forControlEvents:UIControlEventTouchDown];
    add.backgroundColor = [UIColor colorWithWhite:0.92 alpha:1.0];
...
- (void)channelList {
    ...
    if (self.addDelegate && [self.addDelegate respondsToSelector:@selector(didClickAddButton)]) {
        [self.addDelegate didClickAddButton];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    CGFloat bs = scrollView.contentOffset.x / UIScreen.mainScreen.bounds.size.width;
    //eyuxin change
    BOOL delay = bs == (int)bs;
    [self wm_layoutChildViewControllers:delay];
