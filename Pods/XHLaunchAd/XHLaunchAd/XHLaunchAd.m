...
- (void)addLogo:(XHLaunchImageView *)launchImageV {
    CGFloat height = 150;
    UIImage *image = [UIImage imageNamed:@"开屏图"];
    UIImageView *imgV = [[UIImageView alloc] initWithFrame:CGRectMake(0, UIScreen.mainScreen.bounds.size.height - height, KWIDTH, KWIDTH * image.size.height / image.size.width)];
    imgV.image = image;
    imgV.center = CGPointMake(KWIDTH / 2, KHEIGHT - (height / 2));
    [launchImageV addSubview:imgV];
}
...
-(void)setupLaunchAd
{
    ...
    /** 添加launchImageView */
    XHLaunchImageView *launchImageV = [[XHLaunchImageView alloc] init];
    [_window addSubview:launchImageV];
    
    [self addLogo:launchImageV];
...
- (void)removeAndAnimate{
    ...
    [NSNotificationCenter.defaultCenter postNotificationName:@"ADPAGE_BACK" object:nil];
}
